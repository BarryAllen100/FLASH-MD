...
"`, contextInfo: {
                  ...
                  externalAdReply: {
                     ...
                     sourceUrl: 'https://barry-web-rroi.onrender.com/'
                  }
               }
            })
         } else if (action == "remove" && config.WELCOME ) {
           const userName = jid.split('@')[0];
                    const leaveTime = moment.tz('America/Port-au-Prince').format('HH:mm:ss');
                    const leaveDate = moment.tz('America/Port-au-Prince').format('DD/MM/YYYY');
                    const membersCount = metadata.participants.length;
            sock.sendMessage(id, {
